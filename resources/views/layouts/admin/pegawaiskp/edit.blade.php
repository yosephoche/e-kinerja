        ...
        <div class="main-content tab-content">
            <div class="container-fluid">
                <form id="form-update-skp" action="{{$skp->update_uri}}" class="form">
                    <div class="row">
                        <div class="col-md-6">
                            <h2 class="mb-2">Edit Tugas SKP Pegawai</h2>
                            <div class="form-group">
                                {{ $skp->pegawai }}
                                <label for="golongan">List Tugas</label>
                                <select class="form-control" name="skp_task_id" id="SKPD" required>
                                    <option value="">Pilih Tugas SKP</option>
                                    @foreach($skp->skpTask AS $skp_task)

                                        {{-- <option
                                            value="{{$skp->id}}" {{$skp->skp_task_id == $skp_task->id ? 'selected' : ''}}>{{$skp_task->nama}}</option> --}}
                                    @endforeach
                                </select>
                            </div>
                            <hr>
